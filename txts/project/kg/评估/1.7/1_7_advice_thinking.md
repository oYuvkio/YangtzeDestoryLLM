## 文档标题：ABox 评测指标完善与代码修复任务书

### 一、背景说明

当前 `tools/abox_metrics.py` 文件用于评估知识图谱抽取质量，但存在以下问题：
1. 缺少部分关键指标
2. 数据格式处理不完善
3. 部分边界情况未处理
...
**主文件**：`tools/abox_metrics.py`

### 三、修改任务清单

#### 任务1：修改 `_ensure_records` 函数

**目的**：统一处理 `entities` 字段，当输入缺少 `entities` 时从 `triples` 推断

**修改位置**：函数 `_ensure_records`
...
def _ensure_records(obj: Any, *, use_original_type: bool = False) -> Dict[str, List[Dict[str, Any]]]:
    ...
    - 若 obj 是 dict，直接返回其中 events/triples/entities 字段（不存在则空列表或推断）。
    - 若 obj 是 list，假定列表元素为 dict，分别汇总 events/triples/entities。
...
assert len(result["entities"]) == 4  # 长江、东海、洪水、武汉
assert result["entities"][0]["name"] == "长江"
```

---

#### 任务2：新增 `compute_direction_error_rate` 函数

**目的**：检测主宾语颠倒的错误率

**添加位置**：在 `compute_entity_redundancy_rate` 函数之后
...
def compute_direction_error_rate(predictions: Any, gold: Any) -> Dict[str, Any]:
    ...
    返回:
        direction_error_rate: 方向错误率
        direction_errors: 方向错误数量
        total_checked: 检查的三元组总数
    """
    ...
    for pred_t in pred_triples:
        ...
        for gold_t in gold_triples:
            ...
            # 主宾语颠倒检测：predicate 相同，但 subject/object 互换
...
#### 任务3：新增 `compute_partial_match_metrics` 函数

**目的**：计算三元组部分匹配指标

**添加位置**：在 `compute_direction_error_rate` 函数之后
...
#### 任务4：新增 `compute_evidence_quality` 函数

**目的**：评估预测三元组的证据质量

**添加位置**：在 `compute_partial_match_metrics` 函数之后
...
pred = {"triples": [
    ...
    {"subject": "干旱", "predicate": "affects_region", "object": "南京", "evidence": ""},  # 无证据
]}
gold = {"triples": [
    {"subject": "洪水", "predicate": "affects_region", "object": "武汉", "evidence": "1998年洪水影响武汉地区"},
]}
result = compute_evidence_quality(pred, gold)
assert result["evidence_coverage"] == 0.5  # 1/2 有证据
assert result["total_with_evidence"] == 1
```

---

#### 任务5：新增 `compute_event_completeness` 函数

**目的**：评估预测事件的完整性

**添加位置**：在 `compute_evidence_quality` 函数之后
...
# 测试用例
pred = {"events": [
    {"name": "1998年洪水", "event_type": "FloodEvent", "time": {"start_time": "1998-07-01"}, "location": ["武汉"]},
    {"name": "2022年干旱", "event_type": "", "time": {}, "location": []},  # 不完整
]}
result = compute_event_completeness(pred)
assert result["total_events"] == 2
assert result["has_name_rate"] == 1.0
assert result["has_type_rate"] == 0.5
assert result["has_time_rate"] == 0.5
assert result["has_location_rate"] == 0.5
```

---

#### 任务6：新增 `compute_per_class_metrics` 函数

**目的**：计算每种实体类型的 P/R/F1

**添加位置**：在 `compute_event_completeness` 函数之后
...
assert result["geographicregion"]["matched"] == 0
```

---

#### 任务7：修改 `compute_full_metrics` 函数

**目的**：整合所有新增指标

**修改位置**：函数 `compute_full_metrics`

**修改内容**：在函数中添加对新增指标函数的调用，并将结果加入返回字典

**具体修改**：（见上文 5.2 节中的完整实现）

---

#### 任务8：修改 `compute_tbox_consistency` 函数（续）

**目的**：从 entities 字段也推断实体类型，解决 Gold 中 events 为空导致类型推断失败的问题

**修改位置**：函数 `compute_tbox_consistency` 中构建 `name_to_type` 的部分
...
#### 任务9：修改 `main` 函数中的聚合逻辑

**目的**：在批量评测时正确聚合新增指标

**修改位置**：`main` 函数中 `if isinstance(gold, list) and isinstance(preds, list):` 分支
...
def test_ensure_records_with_entities_inference():
    """测试从 triples 推断 entities"""
    test_input = {
        "events": [],
        "triples": [
            {"subject": "长江", "predicate": "flows_into", "object": "东海"},
    ...
    print("✓ test_ensure_records_with_entities_inference passed")

def test_direction_error_rate():
    """测试方向错误检测"""
    pred = {"triples": [{"subject": "洪水", "predicate": "has_hazard_factor", "object": "暴雨"}]}
    gold = {"triples": [{"subject": "暴雨", "predicate": "has_hazard_factor", "object": "洪水"}]}
    result = compute_direction_error_rate(pred, gold)
    assert result["direction_errors"] == 1
    assert result["direction_error_rate"] == 1.0
    print("✓ test_direction_error_rate passed")

def test_partial_match_metrics():
    """测试部分匹配指标"""
    pred = {"triples": [
        {"subject": "洪水", "predicate": "affects_region", "object": "武汉"},
        {"subject": "洪水", "predicate": "affects_region", "object": "南京"},
    ]}
    gold = {"triples": [
        {"subject": "洪水", "predicate": "affects_region", "object": "武汉"},
        {"subject": "洪水", "predicate": "affects_region", "object": "上海"},
    ...
    assert result["breakdown"]["full_match"] == 1
    assert result["breakdown"]["head_relation_match"] == 1
    print("✓ test_partial_match_metrics passed")

def test_evidence_quality():
    """测试证据质量评估"""
    pred = {"triples": [
        {"subject": "洪水", "predicate": "affects_region", "object": "武汉", "evidence": "1998年洪水"},
    ...
    assert result["total_with_evidence"] == 1
    print("✓ test_evidence_quality passed")
...
def test_event_completeness():
    ...
    assert result["has_name_rate"] == 1.0
    assert result["has_type_rate"] == 0.5
    print("✓ test_event_completeness passed")

def test_tbox_consistency_with_entities():
    """测试 TBox 一致性（使用 entities 推断类型）"""
    pred = {
        "entities": [
            {"name": "洪水", "type": "DisasterEvent"},
            {"name": "武汉", "type": "GeographicRegion"},
    ...
    assert stats["domain_range_violations"] == 0
    print("✓ test_tbox_consistency_with_entities passed")

if __name__ == "__main__":
    test_ensure_records_with_entities_inference()
    test_direction_error_rate()
    test_partial_match_metrics()
    test_evidence_quality()
    test_event_completeness()
    test_tbox_consistency_with_entities()
    print("\n所有测试通过！")
```
...
python tools/abox_metrics.py \
    ...
    --pred outputs/eval_models/Qwen_Qwen3-8B/predictions_filtered.jsonl \
...
| 指标名称    | 字段名                                  | 说明                   | 取值范围 |
| ----------- | --------------------------------------- | ---------------------- | -------- |
| 部分匹配 F1 | `partial_match_f1`                      | 加权部分匹配分数       | 0-1      |
| 方向错误率  | `direction_error_rate`                  | 主宾语颠倒的比例       | 0-1      |
| 证据覆盖率  | `evidence_quality.evidence_coverage`    | 有证据的三元组比例     | 0-1      |
| 证据准确率  | `evidence_quality.evidence_accuracy`    | 证据与 Gold 匹配的比例 | 0-1      |
| 事件完整性  | `event_completeness.completeness_score` | 事件四要素平均覆盖率   | 0-1      |
| 分类别 F1   | `per_class_metrics.<type>.f1`           | 每种实体类型的 F1      | 0-1      |
| 分关系 F1   | `per_relation_metrics.<rel>.f1`         | 每种关系的 F1          | 0-1      |
...
1. **Gold 中 events 为空**：这是 Gold 标注策略问题，不是代码 bug。当前修复通过从 `entities` 推断类型来缓解。

2. **中英文混合**：Schema 使用英文名称，但文本是中文。建议在 Prompt 中明确要求输出英文类型名。

3. **主宾语颠倒**：这是模型理解问题，新增的 `direction_error_rate` 指标可以量化这个问题。