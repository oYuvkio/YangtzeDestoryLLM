# 我的方法（论文式写法）：CQ 驱动的长江流域水旱灾害本体/知识图谱构建框架（与原论文逐阶段对比）
...
1) **问题契合**：原论文试图解决“灾害领域知识碎片化 → 难以将专业数据转为可理解/可执行信息”的问题；我的系统同样面临“长江流域水旱灾害材料（年鉴/法规/预案/新闻）分散、术语不统一、难以结构化检索与推理”的现实。

2) **方法吸引点**：原论文的“半自动化 + 人机协同”给了我一个可落地的工程路线：  
- 让 LLM 做“扩展/归纳/抽取”的高吞吐工作；  
- 用规则/评估/人工审核兜底，避免纯自动幻觉；  
- 以“可复现的流水线”替代零散的人工本体编辑。

3) **对我的直接启发**：原论文用 CQ 驱动构建与评估（CQ 覆盖度），这与我“做问答系统”的终端目标天然一致：**CQ 本质上就是系统应该能回答的问题集合**，可作为需求与评价的共同载体。
...
**核心思想（我的复述）**：  
原论文将本体工程拆成可控的 5 个阶段：  
1) 专家定范围与顶层类 → 2) 用 CQ 引导 LLM 扩展 TBox → 3) 用权威文档进一步富集 schema → 4) 用新闻等实例填充 ABox → 5) 用案例验证（把“笼统警报”变成“可执行建议”）。  
...
**我的框架（落地到代码的“P1~P5”）**（核心实现：`kg/cq_pipeline.py`）：  
`P1：领域范围 → 生成 CQ（能力问题）`  
...
|---|---|---|---|
| 阶段划分 | 5 个阶段（本体工程语境） | 5 个阶段（工程流水线语境：P1~P5） | 我需要“可跑脚本 + 可复现输出”的阶段边界；每一阶段必须对应清晰的文件产物 |
...
- P4 中间产物：  
  ...
  - `p4_corpus_suggestions_agg.json`（聚合后的建议，含 `_support`、`_support_sources`）
- P5 输出：`p5_*.json`  
...
**方法描述（更细）**：
1) **写领域范围说明**：用结构化要点描述灾害类型、阶段、致灾因子、影响、脆弱性、措施、典型事件等（示例：`kg/cq_pipeline.py` 中 `DEMO_DOMAIN_DESC`）。  
2) **Prompt 注入“角色 + 约束 + JSON 输出格式”**：使用 `kg/prompts.py` 的 `P1_CQ_PROMPT`，在 Prompt 中明确：  
   - CQ 必须可被结构化查询回答（面向 KG）；  
   - 分类（category）集合与覆盖要求；  
   - 输出必须是合法 JSON，且仅输出 JSON。  
3) **调用 LLM 并强制 JSON**：`kg/cq_pipeline.py` 中 `LLMJsonClient.call()` 将消息组装为 `messages: list[dict]` 并调用 `llm.chat_messages(..., json_mode=True)`。  
4) **鲁棒 JSON 解析**：若模型输出被 ```json 代码块包裹，先去 fence；若仍无法解析，尝试正则截取 `{...}`/`[...]` 片段兜底。  
5) **后处理与落盘**：过滤掉缺少 `question` 的条目，构造 `List[CQ]`，并保存为 `p1_cqs*.json`。
...
class EmbeddingDeduplicator:
    def __init__(self, model_name: str = "BAAI/bge-base-zh-v1.5", threshold: float = 0.75, device: str = "cpu"):
...
6) **输出**：在 `final_dir` 写出多版本 `p4_tbox_augmented_s{support}_allow{0|1}.json`，并可输出冲突报告 `outputs/ontoqa/p4_conflicts_*.json`。
...
1) **构建抽取输入文本（可选上下文）**：  
   ...
   - 通过结构化标记区分抽取目标：  
     - `【待抽取文本】`：主抽取目标  
     - `【前文参考】/【后文参考】`：仅辅助理解  
2) **将 TBox 注入 Prompt，强制 predicate/type 受约束**：  
   - `P5_EXTRACTION_PROMPT` 中明确：  
     - `event_type` 必须来自 `TBox.classes.name`；  
     - `predicate` 必须来自 `TBox.relations.name`；  
3) **调用 LLM（JSON 模式）并拿到 events/triples**：输出结构固定，便于后续评测与入库。  
4) **最小化清洗与一致性校验（关键鲁棒性步骤）**：  
...
5) **可选：实体标准化**：`kg/utils/entity_linking.py` 提供地名/术语别名归一，便于图谱融合与统计。
...
2) **某个 P4 增强版本（展示“增强后 IR 下降”的真实现象）**  
...
我用两类机制代替“专家全量审查”：
1) `kg/utils/conflict_detection.py`：从 TBox 结构角度找“悬空/重复/孤立/空定义”等问题；  
2) `scripts/run_p4_batch.py`：生成 `conflict_report`（例如 parent 冲突、domain/range 冲突），并输出冲突动作日志（用于审计）。
...
1) `kg/cq_pipeline.py:LLMJsonClient` —— 为什么能稳定产出可解析 JSON  
2) `kg/utils/deduplication.py` —— 为什么“阈值 0.7”可以工程化落地并可解释  
3) `scripts/run_p4_batch.py` —— 为什么 P4 能 2~3 小时批量运行且可断点续跑
...
1) 领域说明 → 生成 CQ（P1）  
2) CQ → 初始 TBox + 规范化/去重（P2/P3）  
3) 语料批处理增强 TBox（P4：支持度聚合 + 去重 + 冲突报告）  
4) TBox 约束抽取 events/triples 并清洗（P5）  
5) OntoQA/CQ 覆盖/抽取指标评估，支撑 KG-RAG（Stage5）
...
1) 把“本体构建方法”工程化为 P1~P5 可复现流水线，并形成稳定的中间产物与日志/报告体系。  
2) 通过 `parent` 字段与 OntoQA 工具链，使层级结构（IR）可测量、可对齐论文评价框架。  
3) 设计 P4 的支持度聚合与断点续跑，让“多文档增强”在工程上可运行、可追溯、可对比。  
